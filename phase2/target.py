...
from phase2.utils import (
    distance,
    gravitational_force,
)  # Importation des fonctions utilitaires pour la distance et la gravité
...
class Target(pygame.sprite.Sprite):
    def __init__(self, position, mass, gravity_range, image_path):
        ...
        self.original_image = pygame.image.load(
            image_path
        ).convert_alpha()  # Chargement de l'image avec transparence
        scale_factor = (
            gravity_range / 100
        )  # Calcul du facteur d'échelle basé sur la plage de gravité
        self.image = pygame.transform.scale(
            self.original_image, (int(80 * scale_factor), int(80 * scale_factor))
        )  # Redimensionnement de l'image en fonction du facteur d'échelle
        self.rect = self.image.get_rect(
            center=position
        )  # Création du rectangle de collision de la planète avec son centre à la position donnée
    ...
    def check_collision(self, player):
        ...
        return self.rect.colliderect(
            player.rect
        )  # Vérifie si les rectangles de la cible et du joueur se superposent (collision)

    def draw(self, screen):
        """Affiche la planète cible sur l'écran."""
        screen.blit(
            self.image, self.rect
        )  # Dessine l'image de la planète à sa position sur l'écran
