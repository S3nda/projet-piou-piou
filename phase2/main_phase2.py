...
)  # Définir l'écran en mode plein écran
...
def main_phase2():
    ...
    )  # Créer la fenêtre du jeu avec les dimensions
    pygame.display.set_caption(
        "Void Seekers - Phase 2"
    ...
    background_images = [
        f for f in os.listdir(background_folder) if f.endswith(".png")
    ]  # Liste des images .png
    if not background_images:
        raise FileNotFoundError(
            f"Aucune image trouvée dans le dossier {background_folder}"
        )  # Vérifier si le dossier contient des images

    chosen_background = random.choice(
        background_images
    )  # Choisir une image de fond aléatoire
    background = pygame.image.load(
        os.path.join(background_folder, chosen_background)
    ).convert()  # Charger l'image
    ...
    def is_planet_too_close(new_planet_pos, planets, target_planet):
        ...
        for planet in planets:
            distance_to_planet = pygame.math.Vector2(new_planet_pos).distance_to(
                planet.position
            )
            if distance_to_planet < (
                planet.rect.width / 2 + 100
            ):  # Si la distance est trop petite entre obstacles
                return True
        # Vérification de la distance avec la planète cible
        distance_to_target = pygame.math.Vector2(new_planet_pos).distance_to(
            target_planet.position
        )
        if distance_to_target < (
            target_planet.rect.width / 2 + 100
        ):  # Vérifie si elle est trop proche de la cible
            return True
    ...
    for _ in range(3):  # Créer 3 planètes obstacles
        while True:
            ...
            planet_size = random.choice(
                size_choices
            )  # Choisit aléatoirement une taille parmi les 3 options
    ...
    )  # Dernier temps enregistré pour le calcul du delta time
    ...
    while running:
        ...
        player.update(
            dt, planets
        )  # Mise à jour de la position du joueur et prise en compte des planètes
        ...
        for planet in planets:
            ...
            if distance_to_planet < (player.rect.width / 2 + planet.rect.width / 2):
                ...
                print(
                    "Perdu ! Vous avez percuté une planète obstacle."
                )  # Message de défaite
                running = False
