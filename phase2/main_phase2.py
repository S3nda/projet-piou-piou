...
class Phase2:
    ...
    def generate_planets(self):
        ...
        for _ in range(3):
            while True:
                ...
                if not self.is_planet_too_close(
                    pos, planets
                ) and not self.is_planet_on_player(pos, planets):
                    planet = Planet(
                        position=pos,
    ...
    def is_planet_too_close(self, new_pos, planets):
        for planet in planets:
            if pygame.math.Vector2(new_pos).distance_to(planet.position) < (
                planet.rect.width / 2 + 100
            ):
                return True
        if pygame.math.Vector2(new_pos).distance_to(self.target_planet.position) < (
            self.target_planet.rect.width / 2 + 100
        ):
            return True
        return False

    def is_planet_on_player(self, new_pos, planets):
        if pygame.math.Vector2(new_pos).distance_to(self.player.position) < (
            self.player.rect.width / 2 + 100
        ):
            return True
        if pygame.math.Vector2(new_pos).distance_to(self.target_planet.position) < (
            self.target_planet.rect.width / 2 + 100
    ...
    def check_victory(self):
        ...
        return distance < (
            self.player.rect.width / 2 + self.target_planet.rect.width / 2
        )
